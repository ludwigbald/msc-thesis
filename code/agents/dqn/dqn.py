...
Adapted for the CityLearn environment:
    - multi-agent environment, but train a single model
...
class DQN:
    ...
    def learn(self, timesteps, verbose=False):
        ...
        for timestep in range(timesteps):
            ...
            states_next, rewards, done, _ = self.env.step(actions)
            states_next = [torch.Tensor(s) for s in states_next]

            # Store transition in replay buffer
            for action, reward, state, state_next in zip(actions, rewards, states, states_next):
