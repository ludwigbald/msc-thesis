...
def get_reward(electricity_consumption: List[float], carbon_emission: List[float], electricity_price: List[float], agent_ids: List[int]) -> List[float]:
        ...
        total_cost_no_battery = np.array([2250.8700563860034, 1966.1250130131689, 1315.5171066205235, 1707.2781431365536, 1540.8814011827963]) /8760
        total_co2_no_battery = np.array([1117.6211690517193, 1043.3168421031762, 707.0223402007878, 967.9933801771755, 791.53366612883]) /8760

        carbon_emission = np.array(carbon_emission).clip(min=0) / total_co2_no_battery#.mean()
        electricity_price = np.array(electricity_price).clip(min=0) /total_cost_no_battery#.mean()
