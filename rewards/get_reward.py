...
def get_reward(electricity_consumption: List[float], carbon_emission: List[float], electricity_price: List[float], agent_ids: List[int]) -> List[float]:
        ...
        carbon_emission = np.array(carbon_emission).clip(min=0)/total_co2_no_battery - 1
        electricity_price = np.array(electricity_price).clip(min=0)/total_cost_no_battery -1
        reward = (carbon_emission + electricity_price)*-1
        #reward = (electricity_price)*-1
        # ************** END ***************
        return reward